...
PLIST_SRC="$REPO_DIR/scripts/com.sloshbot.daily.plist"
PLIST_DST="$HOME/Library/LaunchAgents/com.sloshbot.daily.plist"
...
launchctl bootout "gui/$UID_NUM/com.sloshbot.daily" 2>/dev/null || true
launchctl bootstrap "gui/$UID_NUM" "$PLIST_DST"

echo "Installed com.sloshbot.daily -> $PLIST_DST"
echo "Scheduled to run daily at 7:30am (missed firings while asleep catch up on wake)."
echo
echo "Trigger a manual run right now:"
echo "  launchctl kickstart gui/$UID_NUM/com.sloshbot.daily"
echo
